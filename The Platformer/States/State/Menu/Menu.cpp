...
void Menu::InitButtons() {
	...
	{
		{ "start",	  Button("JOUER",	  sf::Vector2f(WINDOW_WIDTH, WINDOW_HEIGHT) / 2.0f, m_FontManager.Get("default")) },
		{ "settings", Button("OPTIONS",  sf::Vector2f(WINDOW_WIDTH / 2.0f, 550   ),        m_FontManager.Get("default")) },
		{ "exit",	  Button("QUITTER",	  sf::Vector2f(WINDOW_WIDTH / 2.0f, 650   ),        m_FontManager.Get("default")) },
	};
...
void Menu::InitTexts() {
	// TITLE
	{
		...
		t.setString("NIGHTMARE SURVIVAL !");
...

	{
		...
		t.setString("MEILLEUR TEMPS: " + ss.str());
